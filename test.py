# Runs a few tests to check if the code executes correctly
...
class run_tests(unittest.TestCase):
    ...
    def test_send_message(self):
        ...
        for node in neighbors[A.id_number]:
            assert [0, ["code_fragment_1_version_2", "code_fragment_2_version_2"], [
                True, True]] in node.messages
    ...
    def test_act(self):
        ...
        A.send_message(neighbors)
        assert A.c == 0
        B.act(False, neighbors)  # We are at t < tau
        assert B.ld == A.ld
        assert B.md == A.md
        assert B.c == 0
        assert B.messages == []
        # B sent a message to his neighbors
        C.act(False, neighbors)
        assert C.ld == B.ld
        assert C.md == B.md
        assert C.c == 0
        assert C.messages == []
        # Same thing with C
        D.act(False, neighbors)
        assert D.ld == A.ld
        assert D.md == A.md
        assert D.c == 0
        assert D.messages == []
        # Same with D
        E.act(False, neighbors)
        assert E.ld == D.ld and E.ld == B.ld and E.ld == C.ld
        assert E.md == D.md and E.md == B.md and E.md == C.md
        assert E.c == 4
        assert E.messages == []
        # Same : E send a message
        A.act(True, neighbors)
        assert A.c == 0
        assert A.i == 2
        assert A.messages == []
        assert A.t == 0
        assert A.i / 2 <= A.tau <= A.i
        # to make another update, but after tau this time
        B.md = [False, False]
        A.send_message(neighbors)
        B.act(True, neighbors)
        assert B.messages == []
